...
FIXED VERSION - addresses common deployment and runtime issues.
"""
...
def get_scraper_mapping(_organizations_data):
    ...
    try:
        ...
        print(f"Error creating scraper mapping: {e}")
...
def get_scraper_paths_by_type(_organizations_data, scraper_type):
    ...
    try:
        ...
        print(f"Error getting scraper paths: {e}")
...
def send_slack_notification(failed_scrapers, is_daily_report=False):
    ...
    try:
        ...
        for scraper in failed_scrapers[:10]:  # Limit to first 10 to avoid message too long
            school = scraper.get('School', 'Unknown')
            name = scraper.get('Name', 'Unknown')
            reason = scraper.get('Health Reason', 'Unknown error')
        ...
        if len(failed_scrapers) > 10:
            message += f"... and {len(failed_scrapers) - 10} more\n\n"
...
def check_and_send_daily_report(mongo_uri, db_name, _organizations_data):
...
def display_announcements():
    ...
    try:
        ...
        if not organizations_data:
            st.error("Could not load organizations data. Please check database connection.")
            return
        ...
        # Create unique filter state key
            ...
            if num_announcements > 0:
                if st.button("📥 Generate CSV", help="Click to generate and download CSV (may take a moment for large datasets)"):
                    with st.spinner("Generating CSV file..."):
                        try:
                            # For CSV generation, we need all data
                            csv_data = get_all_announcements_for_csv(MONGO_URI, DB_NAME, query, scraper_mapping)
                            if csv_data:
                                st.download_button(
                                    label="📥 Download CSV",
                                    data=csv_data,
        ...
        for ann in paged_announcements:
            try:
                ...
                <p style="margin-bottom: 0.5em;">
                    ...
                    <strong>Announcement URL:</strong><br/> <a href="{url}" target="_blank">{url}</a>
                </p>
                """
                ...
                if search_term.strip() and content:
                    try:
                        ...
                        print(f"Error processing search snippet: {e}")
                ...
                st.error(f"Error displaying announcement: {e}")
                continue
        ...
        st.error(f"Error in display_announcements: {e}")
        st.error(f"Traceback: {traceback.format_exc()}")

@st.cache_data(ttl=60)
def get_all_announcements_for_csv(mongo_uri, db_name, query, scraper_mapping):
    """Get all announcements for CSV generation"""
    try:
        client = MongoClient(mongo_uri, serverSelectionTimeoutMS=5000)
        db = client[db_name]
        
        cursor = db.articles.find(query, {"_id": 0}).sort("date", -1).limit(10000)  # Limit for safety
        ...
        return convert_to_csv(announcements, scraper_mapping)
    except Exception as e:
...
def get_scraper_status_data(mongo_uri, db_name, _organizations_data):
    ...
    try:
        ...
        print(f"Error getting scraper status data: {e}")
...
def display_scraper_status():
    ...
    try:
        ...
        if SLACK_WEBHOOK_URL:
            try:
                sent, message = check_and_send_daily_report(MONGO_URI, DB_NAME, organizations_data)
        ...
        all_scrapers_data, health_counts, failed_scrapers = get_scraper_status_data(MONGO_URI, DB_NAME, organizations_data)
        ...
        if all_scrapers_data:
            ...
            st.warning("No scraper data available.")
            
    except Exception as e:
        st.error(f"Error in display_scraper_status: {e}")
        st.error(f"Traceback: {traceback.format_exc()}")
...
def get_schools_summary_data(mongo_uri, db_name, _organizations_data, start_date):
    ...
    try:
        ...
        print(f"Error getting schools summary data: {e}")
        return []

def display_schools_summary():
    ...
    try:
        ...
        if not schools_data:
            st.error("Could not load schools summary data.")
            return
        ...
        st.error(f"Error in display_schools_summary: {e}")
        st.error(f"Traceback: {traceback.format_exc()}")
...
def main():
    ...
    try:
        ...
        st.markdown("Announcements from offices at select universities.")
        ...
        try:
            ...
            st.info("Please check your MongoDB connection settings and try refreshing the page.")
            st.stop()
        ...
        with tab3:
            display_schools_summary()
            
    except Exception as e:
        st.error(f"❌ Application error: {e}")
        st.error(f"Detailed traceback: {traceback.format_exc()}")
        st.info("Please try refreshing the page. If the problem persists, check the application logs.")
